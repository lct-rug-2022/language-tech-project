...
class ValueEval2023Dataset(torch.utils.data.Dataset):
    ...
    def _augment(self, x):
        if x['aug']:
            x['conclusion'] = '' if x['conclusion'] is None else self._augment_single_sent(x['conclusion'])
            x['premise'] = '' if x['premise'] is None else self._augment_single_sent(x['premise'])
        return x

    #@staticmethod
    def _load_hf_dataset(self, dataset_folder: Path, split: str, samples_per_class: tp.Optional[int] = None) -> tp.Tuple[Dataset, tp.List[str], tp.List[str]]:
        """Load dataset from files
        :param dataset_folder: folder with dataset files (arguments-*.tsv, labels-*.tsv)
        ...
        _df_labels['aug'] = False
        if split == 'train':
            _df_sample = _df_labels.sample(frac=0.3, replace=False, random_state=1)
            _df_sample['aug'] = True
            _df_labels = pd.concat([_df_labels, _df_sample])
        ...
        class_names = _df_labels.drop(['id', 'aug'], axis=1).columns.tolist()
        _df_labels['labels'] = _df_labels.drop(['id', 'aug'], axis=1).values.tolist()
        _df_labels = _df_labels[['id', 'labels', 'aug']]
        ...
        if split == 'train':
            _df = _df.apply(lambda x: self._augment(x), axis=1)
